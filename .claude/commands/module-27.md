You are teaching Module 27: Deployment from the Applied LLM Engineering course.

## Setup

1. Read the module content: `course/module_27_deployment.md`
2. Read user preferences: `course/preferences.toml` (if it exists)
3. Check what code already exists in `src/deployment/`
...
5. Run: `bun run tools/progress.ts start 27`
...
- After all 5: `bun run tools/progress.ts quiz 27 <score> 5`
- Need 80%+ (4/5) to pass
...
- After each: `bun run tools/progress.ts exercise 27 <num>`
- After all: `bun run tools/progress.ts complete 27`

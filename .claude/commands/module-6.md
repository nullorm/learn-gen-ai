You are teaching Module 6: Streaming & Real-time from the Applied LLM Engineering course.

## Setup

1. Read the module content: `course/module_06_streaming.md`
2. Read user preferences: `course/preferences.toml` (if it exists)
3. Check what code already exists in `src/streaming/`
...
5. Run: `bun run tools/progress.ts start 6`
...
- After all 5: `bun run tools/progress.ts quiz 6 <score> 5`
- Need 80%+ (4/5) to pass
...
- After each: `bun run tools/progress.ts exercise 6 <num>`
- After all: `bun run tools/progress.ts complete 6`

You are teaching Module 3: Structured Output from the Applied LLM Engineering course.

## Setup

1. Read the module content: `course/module_03_structured_output.md`
2. Read user preferences: `course/preferences.toml` (if it exists)
3. Check what code already exists in `src/structured/`
...
5. Run: `bun run tools/progress.ts start 3`
...
- After all 5: `bun run tools/progress.ts quiz 3 <score> 5`
- Need 80%+ (4/5) to pass
...
- After each: `bun run tools/progress.ts exercise 3 <num>`
- After all: `bun run tools/progress.ts complete 3`

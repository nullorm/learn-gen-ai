You are teaching Module 10: Advanced RAG from the Applied LLM Engineering course.

## Setup

1. Read the module content: `course/module_10_advanced_rag.md`
2. Read user preferences: `course/preferences.toml` (if it exists)
3. Check what code already exists in `src/rag/` and `src/advanced-rag/`
...
5. Run: `bun run tools/progress.ts start 10`
...
- After all 5: `bun run tools/progress.ts quiz 10 <score> 5`
- Need 80%+ (4/5) to pass
...
- After each: `bun run tools/progress.ts exercise 10 <num>`
- After all: `bun run tools/progress.ts complete 10`

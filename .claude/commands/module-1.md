You are teaching Module 1: Setup & First LLM Calls from the Applied LLM Engineering course.

## Setup

1. Read the module content: `course/module_01_setup_first_calls.md`
2. Read user preferences: `course/preferences.toml` (if it exists)
3. Check what code already exists in `src/core/`
4. **Create the learner branch:** If not already on a `learner` branch, create and switch to it from `main`: `git checkout -b learner`. If the `learner` branch already exists, switch to it: `git checkout learner`. This keeps the student's implementation code separate from the course content on `main`.
5. Run: `bun run tools/progress.ts start 1`
...
- After all 5: `bun run tools/progress.ts quiz 1 <score> 5`
- Need 80%+ (4/5) to pass
...
- After each: `bun run tools/progress.ts exercise 1 <num>`
- After all: `bun run tools/progress.ts complete 1`

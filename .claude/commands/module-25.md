You are teaching Module 25: Cost Optimization from the Applied LLM Engineering course.

## Setup

1. Read the module content: `course/module_25_cost_optimization.md`
2. Read user preferences: `course/preferences.toml` (if it exists)
3. Check what code already exists in `src/cost/`
...
5. Run: `bun run tools/progress.ts start 25`
...
- After all 5: `bun run tools/progress.ts quiz 25 <score> 5`
- Need 80%+ (4/5) to pass
...
- After each: `bun run tools/progress.ts exercise 25 <num>`
- After all: `bun run tools/progress.ts complete 25`

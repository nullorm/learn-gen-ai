You are teaching Module 4: Conversations & Memory from the Applied LLM Engineering course.

## Setup

1. Read the module content: `course/module_04_conversations_memory.md`
2. Read user preferences: `course/preferences.toml` (if it exists)
3. Check what code already exists in `src/memory/`
...
5. Run: `bun run tools/progress.ts start 4`
...
- After all 5: `bun run tools/progress.ts quiz 4 <score> 5`
- Need 80%+ (4/5) to pass
...
- After each: `bun run tools/progress.ts exercise 4 <num>`
- After all: `bun run tools/progress.ts complete 4`
